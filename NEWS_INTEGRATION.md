...
При загрузке новостей из VK (`news/vk.py`) и из CSV (`news/io.py`) после парсинга по умолчанию включается **пост-фильтр по релевантности Ozon** (`NEWS_POST_PARSE_OZON_FILTER`: `True` в `config.py`). Оставляются только строки, где ключевые слова Ozon встречаются в `title` или `text` (логика в `news/ozon_filter.py`). Чтобы брать все строки без этого шага, выставьте `"NEWS_POST_PARSE_OZON_FILTER": False`.
...
## 3. Два датасета и объединение в основном пайплайне

В эксперименте участвуют **два независимых входа** (разные файлы в `config.py`):


| Источник  | Назначение                                                                                                               |
| --------- | ------------------------------------------------------------------------------------------------------------------------ |
| Котировки | `FILE_PATH` — дневной ряд OHLCV, загрузка и валидация в `run_experiment`.                                                |
| Новости   | При `USE_NEWS_SENTIMENT=True` — `NEWS_CSV_PATH`: построчные новости с датой и текстом (и при необходимости `sentiment`). |


**Объединение:** сначала по котировкам считаются `log_ret` и **технические индикаторы** (`compute_indicators`). Затем `attach_news_sentiment_features` подтягивает новости из CSV, агрегирует их **по календарным дням** в признаки и **присоединяет колонки к тому же дневному индексу**, что и котировки. Получается **единая таблица** «цены + индикаторы + новостные признаки дня»; все последующие шаги (**EDA**, **ARIMA–GARCH**, **ML**) выполняются уже над этим объединённым набором. Если `USE_NEWS_SENTIMENT=False`, второй файл не используется, работа идёт только с котировками и индикаторами.

## 4. Как признаки попадают в матрицу `X`

1. После объединения (см. выше) к ряду добавлены колонки
  `news_count`, `has_news`, `sentiment_score`, `sentiment_trend_3`, `sentiment_volatility` (при включённых новостях).
